...
{
    class GatewayDisconnect : EndPointPostAbstract
    {
        ...
        GatewayDisconnect(string netaddress)
        {
            ...
            {
                Address = $"http://localhost:9980/gateway/disconnect/{netaddress}";
            }
            else
            {
        ...
        public GatewayDisconnect(string netaddress, string siaAddress)
        {
            Address = null;
            //regex matches for 1 to 3 digits with period seperator, will throw exception if ip address is not valid  Addresses need to have port numbers as well.
            if (this.IsValidNetAddress(netaddress))
            {
                ...
                Address = $"http://{siaAddress}/gateway/disconnect/{_netaddress}";
            }
            else
            {
                throw new ArgumentException();
        ...
        public async Task Post(HttpClient client)
        {
            ...
            {
                HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, $"/gateway/disconnect/{_netaddress}");
